...
async function load() {
  ...
  const requests = await Promise.allSettled([
    ...
    api.get('/channel/operations/card-templates', { params: { title: 'AgentHub', text: tx('任务处理完成', 'Task completed') } }),
  ])
...
async function testChannel() {
  ...
  try {
    ...
    const response = await api.post(`/workspace/channel/${selected.value.id}/test`, { message: 'AgentHub channel connectivity test' }) as any
    if (response.code !== 200) throw new Error(response.message)
