...
<template>
  <div class="console-page governance-page" data-no-ui-translate v-loading="loading">
    ...
    <section v-else-if="activeTab === 'data'" class="governance-workspace">
      ...
      <StructuredResult v-if="compliance" :data="compliance" :raw-label="tx('查看原始 JSON', 'View raw JSON')" />
    </section>
    ...
    <aside v-if="result" class="governance-result"><header><span>{{ tx('最近执行结果', 'Latest execution result') }}</span><button aria-label="Close" @click="result = null">×</button></header><StructuredResult :data="result" :raw-label="tx('查看原始 JSON', 'View raw JSON')" /></aside>
  </div>
</template>
...
.governance-summary span { color: var(--console-muted); font-size: 12px; }.governance-summary strong { color: var(--console-ink); font: 22px ui-monospace, monospace; }.governance-summary strong.alert { color: var(--console-danger); }
...
form h3 { margin: 0 0 3px; color: var(--console-ink); font-size: 12px; }.form-note { margin: 0; color: var(--console-muted); font-size: 12px; line-height: 1.6; }.field-row { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 8px; }
