...
const packageForm = ref({
  ...
  sourceUri: 'registry://private/sample.echo/1.0.0', artifact: 'AgentMesh signed sample package',
...
<template>
  <div class="console-page ecosystem-page" data-no-ui-translate v-loading="loading">
    ...
    <section v-else class="ecosystem-workspace">
      ...
      <div class="portal-spec"><StructuredResult :data="portal" :raw-label="tx('查看原始 JSON', 'View raw JSON')" /><button class="console-primary" @click="downloadHealth"><el-icon><Download /></el-icon>{{ tx('下载脱敏报告', 'Download redacted report') }}</button></div>
    </section>

    <aside v-if="result" class="ecosystem-result"><header><span>{{ tx('最近执行结果', 'Latest execution result') }}</span><button aria-label="Close" @click="result = null">×</button></header><StructuredResult :data="result" :raw-label="tx('查看原始 JSON', 'View raw JSON')" /></aside>
