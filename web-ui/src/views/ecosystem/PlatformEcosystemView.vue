...
<template>
  <div class="console-page ecosystem-page" data-no-ui-translate v-loading="loading">
    ...
    <section v-else-if="activeTab === 'multimodal'" class="ecosystem-workspace">
      <header><div><h2>{{ tx('多模态与结构化抽取', 'Multimodal and structured extraction') }}</h2><p>{{ tx('本地解析基础结构；开启语义理解后，阿里云百炼 Qwen-VL 与 Qwen Audio ASR 执行图片 OCR、视觉理解和音频转写。', 'Extracts basic structure locally; with semantic understanding enabled, Alibaba Cloud Qwen-VL and Qwen Audio ASR perform image OCR, visual understanding, and audio transcription.') }}</p></div><el-icon><MagicStick /></el-icon></header>
