...
<template>
  <div class="auth-page">
    <section class="auth-visual">
      ...
      <header><button aria-label="返回官网" @click="router.push('/')"><el-icon><ArrowLeft /></el-icon></button><img src="/bg.svg" alt="AgentHub" /><strong>AgentHub</strong><PersonalizationPanel class="auth-personalization" /></header>
    ...
    <main class="auth-panel">
      <div class="auth-panel__inner">
        <header class="mobile-brand"><img src="/bg.svg" alt="AgentHub" /><strong>AgentHub</strong><PersonalizationPanel compact /><button @click="router.push('/')"><el-icon><ArrowLeft /></el-icon></button></header>
        ...
        <div class="entry-heading"><span>{{ mode === 'login' ? 'WELCOME BACK' : 'CREATE WORKSPACE ACCOUNT' }}</span><h2>{{ mode === 'login' ? '进入 AgentHub' : '创建企业账号' }}</h2><p>{{ mode === 'login' ? '使用组织账号访问 Agent 控制台与工作台。' : '创建本地账号，并记录团队与应用场景偏好。' }}</p></div>
        ...
        <form v-else class="auth-form register-form" @submit.prevent="handleRegister">
          ...
          <label><span>企业 / 团队名称 *</span><div><el-icon><OfficeBuilding /></el-icon><input v-model.trim="registerForm.teamName" placeholder="例如：AgentHub 演示团队" /></div></label>
...
<style scoped>
.auth-page { min-height: 100vh; display: grid; grid-template-columns: 3fr 2fr; overflow: hidden; background: #090c0f; color: #edf3ef; font-family: Inter, "PingFang SC", "Microsoft YaHei", system-ui, sans-serif; }.auth-visual { position: relative; min-width: 0; min-height: 100vh; padding: 34px clamp(30px,5vw,70px); display: flex; flex-direction: column; overflow: hidden; border-right: 1px solid #293137; background: #070a0d; }.auth-visual > header { position: relative; z-index: 2; display: flex; align-items: center; gap: 9px; }.auth-visual > header button { width: 32px; height: 32px; margin-right: 5px; display: grid; place-items: center; border: 1px solid #30383e; border-radius: 6px; background: #111518; color: #7c878e; cursor: pointer; }.auth-visual > header img { width: 31px; height: 31px; }.auth-visual > header strong { font-size: 14px; }.auth-visual > header > span { margin-left: 4px; padding: 4px 7px; display: flex; align-items: center; gap: 5px; border: 1px solid #31513f; border-radius: 10px; color: #6fe0a5; font: 6px ui-monospace, monospace; }.auth-visual > header > span i { width: 5px; height: 5px; border-radius: 50%; background: #5ee19f; box-shadow: 0 0 8px rgba(94,225,159,.6); }.auth-node { position: absolute; width: 8px; height: 8px; border: 1px solid #3f6652; border-radius: 50%; background: #111a15; box-shadow: 0 0 13px rgba(81,220,151,.28); }.node-1 { left: 10%; top: 24%; }.node-2 { right: 13%; top: 16%; }.node-3 { right: 22%; bottom: 12%; }.auth-link { position: absolute; height: 1px; background: #1e3027; transform-origin: left center; }.link-1 { left: 10%; top: 24%; width: 78%; transform: rotate(-6deg); }.link-2 { right: 21%; top: 16%; width: 30%; transform: rotate(84deg); }.auth-message { position: relative; z-index: 2; width: min(640px,100%); margin: auto 0; }.auth-message h1 { margin-top: 17px; font-size: clamp(38px,4.2vw,56px); line-height: 1.08; }.auth-message > p { max-width: 600px; margin-top: 18px; color: #7f8b92; font-size: 12px; line-height: 1.75; }.auth-terminal { margin-top: 30px; border: 1px solid #2e373c; border-radius: 8px; background: rgba(10,14,16,.93); box-shadow: 0 25px 60px rgba(0,0,0,.42); overflow: hidden; }.auth-terminal > header { min-height: 40px; padding: 0 12px; display: grid; grid-template-columns: 1fr auto 1fr; align-items: center; border-bottom: 1px solid #283035; }.auth-terminal > header div { display: flex; gap: 5px; }.auth-terminal > header div i { width: 7px; height: 7px; border-radius: 50%; background: #4a5359; }.auth-terminal > header div i:first-child { background: #e57167; }.auth-terminal > header div i:nth-child(2) { background: #dcae56; }.auth-terminal > header div i:last-child { background: #52d894; }.auth-terminal > header span { color: #59646b; font: 7px ui-monospace, monospace; }.auth-terminal > header b { justify-self: end; color: #5cd99b; font: 6px ui-monospace, monospace; }.auth-terminal > div { padding: 13px 15px; }.auth-terminal p { min-height: 28px; display: grid; grid-template-columns: 70px 58px 1fr 8px; align-items: center; color: #69747b; font: 7px ui-monospace, monospace; transition: color .2s; }.auth-terminal p.active { color: #aab6af; }.auth-terminal p > b { color: #68dca1; }.auth-terminal p.cyan > b { color: #63c6cc; }.auth-terminal p.amber > b { color: #deb45a; }.auth-terminal p.blue > b { color: #6eb7e1; }.auth-terminal p strong { font-weight: 500; }.auth-terminal p > i { width: 5px; height: 11px; background: #5ddf9e; animation: caretBlink .8s infinite; }.auth-terminal > footer { min-height: 38px; padding: 0 14px; display: flex; align-items: center; justify-content: space-between; border-top: 1px solid #283035; color: #59646b; font: 6px ui-monospace, monospace; }.auth-terminal > footer b { display: flex; align-items: center; gap: 5px; color: #619577; }.auth-terminal > footer b i { width: 5px; height: 5px; border-radius: 50%; background: #57d998; }.auth-visual > footer { position: relative; z-index: 2; display: flex; flex-wrap: wrap; gap: 22px; color: #647078; font-size: 8px; }.auth-visual > footer span { display: flex; align-items: center; gap: 6px; }.auth-visual > footer .el-icon { color: #65d99f; }
.auth-panel { min-width: 0; min-height: 100vh; padding: 36px clamp(26px,4vw,54px); display: flex; align-items: center; justify-content: center; background: #0c1013; overflow-y: auto; }.auth-panel__inner { width: min(440px,100%); }.mobile-brand { display: none; }.auth-mode { padding: 4px; display: grid; grid-template-columns: 1fr 1fr; border: 1px solid #2d353b; border-radius: 7px; background: #0a0e10; }.auth-mode button { min-height: 38px; border: 0; border-radius: 5px; background: transparent; color: #707b82; font: inherit; font-size: 9px; font-weight: 700; cursor: pointer; }.auth-mode button.active { background: #1b2420; color: #7ce4ad; box-shadow: inset 0 0 0 1px #344a3e; }.entry-heading { margin: 29px 0 21px; }.entry-heading > span { color: #5fdda0; font: 7px ui-monospace, monospace; }.entry-heading h2 { margin-top: 8px; font-size: 27px; }.entry-heading p { margin-top: 7px; color: #747f86; font-size: 9px; }.sso-actions { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; }.sso-actions button { min-height: 43px; padding: 0 10px; display: grid; grid-template-columns: 22px 1fr auto; gap: 7px; align-items: center; border: 1px solid #30383e; border-radius: 6px; background: #121619; color: #b7c1bb; font: inherit; text-align: left; cursor: pointer; }.sso-actions button:hover { border-color: #435049; background: #151b18; }.sso-actions button .el-icon { color: #70b9e2; }.sso-actions button > b { color: #bdc7c1; font: 8px ui-monospace, monospace; }.sso-actions button span { font-size: 9px; }.sso-actions button small { color: #5f6970; font-size: 7px; }.auth-divider { min-height: 48px; display: grid; grid-template-columns: 1fr auto 1fr; gap: 9px; align-items: center; color: #576168; font-size: 7px; }.auth-divider i { height: 1px; background: #293137; }.auth-form { display: flex; flex-direction: column; gap: 13px; }.auth-form label > span, .label-row > span { display: block; margin-bottom: 6px; color: #879198; font-size: 8px; font-weight: 700; }.label-row { display: flex; align-items: center; justify-content: space-between; }.label-row small { margin-bottom: 6px; color: #59646b; font-size: 7px; }.auth-form label > div:not(.label-row) { min-height: 43px; padding: 0 11px; display: flex; align-items: center; gap: 8px; border: 1px solid #30383e; border-radius: 6px; background: #090d0f; }.auth-form label > div:focus-within { border-color: #4b805f; box-shadow: 0 0 0 3px rgba(81,221,151,.06); }.auth-form label .el-icon { color: #69747b; }.auth-form input, .auth-form select { min-width: 0; flex: 1; border: 0; outline: 0; background: transparent; color: #dbe3de; font: inherit; font-size: 9px; }.auth-form select { width: 100%; }.auth-form label button { width: 28px; height: 28px; display: grid; place-items: center; border: 0; background: transparent; color: #667178; cursor: pointer; }.field-row { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; }.password-strength { display: flex; align-items: center; justify-content: space-between; }.password-strength > span { width: 80%; display: grid; grid-template-columns: repeat(4,1fr); gap: 4px; }.password-strength i { height: 3px; border-radius: 2px; background: #283035; }.password-strength i.active { background: #59dc9b; }.password-strength b { color: #69747b; font-size: 7px; }.form-error { padding: 9px 10px; border: 1px solid #55302c; border-radius: 5px; background: #241413; color: #e1877e; font-size: 8px; line-height: 1.5; }.auth-submit { min-height: 45px; padding: 0 14px; display: flex; align-items: center; justify-content: space-between; border: 1px solid #55df9c; border-radius: 6px; background: #53de99; color: #06120a; font: inherit; font-size: 10px; font-weight: 800; cursor: pointer; box-shadow: 0 0 24px rgba(75,222,148,.12); }.auth-submit:disabled { opacity: .5; }.demo-account { min-height: 38px; display: flex; align-items: center; justify-content: space-between; border-top: 1px solid #293137; }.demo-account span { color: #626d74; font-size: 7px; }.demo-account button { padding: 0; border: 0; background: transparent; color: #67daa0; font: inherit; font-size: 7px; cursor: pointer; }.auth-security { margin-top: 21px; padding-top: 16px; display: flex; align-items: center; gap: 9px; border-top: 1px solid #293137; color: #61d99d; }.auth-security p { display: flex; flex-direction: column; gap: 4px; }.auth-security strong { color: #b9c3bd; font-size: 8px; }.auth-security small { color: #5f6970; font-size: 7px; }
