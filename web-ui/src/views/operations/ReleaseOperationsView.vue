...
async function loadAll() {
  ...
  const requests = await Promise.allSettled([
    ...
    api.get('/product/traces'), api.get('/product/traces/summary'),
  ...
  endpoints.value = data[5] || []
  diagnostics.value = data[6] || { summary: {}, checks: [] }
  knowledgeSources.value = data[7] || []
  knowledgeRuns.value = data[8] || []
  templates.value = data[9] || []
  triggers.value = data[10] || []
  workflows.value = data[11] || []
...
<template>
  <div class="console-page release-page" v-loading="loading">
    ...
    <div class="ops-tabs" role="tablist">
      <button v-for="tab in [
        ['versions', tx('版本与评测', 'Versions & evals')], ['traces', 'Trace'], ['routing', tx('模型路由', 'Model routing')],
    ...
    <section v-else-if="activeTab === 'traces'" class="ops-band">
      <header><div><span>03</span><h2>{{ tx('完整会话 Trace', 'End-to-end conversation traces') }}</h2></div><small>{{ tx('模型、工具、检索、审批、Token 与费用', 'Model, tools, retrieval, approvals, tokens and cost') }}</small></header>
