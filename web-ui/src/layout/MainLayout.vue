...
const pageTitle = computed(() => locale.value === 'en-US'
  ? englishPageTitles[String(route.name)] || 'AgentMesh'
  : (route.meta.title as string) || 'AgentMesh')
...
function notifyNewApprovals(count: number) {
  ...
  if (previousPendingApprovals > 0 && preferences.desktopNotifications && typeof Notification !== 'undefined' && Notification.permission === 'granted') {
    new Notification(locale.value === 'en-US' ? 'AgentMesh approval required' : 'AgentMesh 有新的审批待办', {
  ...
  <div class="console-shell">
    <aside :class="['console-sidebar', { collapsed, mobile: mobileOpen }]">
      <div class="console-brand-row">
        <button class="console-brand" aria-label="AgentMesh 首页" @click="navigate('/console/dashboard')">
          <span class="brand-mark"><i /><i /><b /></span>
          <span v-if="!collapsed" class="brand-copy"><strong>AgentMesh</strong><small>CONTROL PLANE</small></span>
        </button>
    ...
    <section class="console-main">
      <header class="console-topbar">
        <div class="topbar-leading">
          ...
          <div class="breadcrumb"><span>AgentMesh</span><i>/</i><strong>{{ pageTitle }}</strong></div>
        </div>
...
.brand-copy { min-width: 0; display: flex; flex-direction: column; gap: 3px; }.brand-copy strong { color: #f5f8f6; font-size: 14px; }.brand-copy small { color: #626a70; font: 12px ui-monospace, monospace; letter-spacing: .08em; }
...
.workspace-avatar { width: 32px; height: 32px; display: grid; place-items: center; border-radius: 6px; background: #24392f; color: #85efbd; font: 12px ui-monospace, monospace; font-weight: 800; }.workspace-switcher > span:nth-child(2) { min-width: 0; display: flex; flex-direction: column; gap: 4px; }.workspace-switcher strong { overflow: hidden; font-size: 12px; text-overflow: ellipsis; white-space: nowrap; }.workspace-switcher small { display: flex; align-items: center; gap: 5px; color: #707981; font-size: 12px; }.workspace-switcher small i { width: 6px; height: 6px; border-radius: 50%; background: var(--console-danger); }.workspace-switcher small i.up { background: var(--console-accent); box-shadow: 0 0 8px rgba(87,229,161,.55); }.workspace-switcher small i.checking { background: var(--console-warning); }.workspace-switcher kbd { padding: 3px 5px; border: 1px solid #30363b; border-radius: 4px; color: #656e75; font: 12px ui-monospace, monospace; }
.console-navigation { flex: 1; padding: 4px 8px 18px; overflow-y: auto; scrollbar-width: thin; scrollbar-color: #2c3237 transparent; }.nav-group { margin-top: 15px; }.nav-group__label { display: block; padding: 0 10px 6px; color: #515960; font-size: 12px; font-weight: 700; }.nav-group button { position: relative; width: 100%; min-height: 38px; padding: 0 10px; display: flex; align-items: center; gap: 10px; border: 0; border-radius: 6px; background: transparent; color: #858e95; font: inherit; font-size: 12px; text-align: left; cursor: pointer; }.nav-group button::before { content: ''; position: absolute; left: 0; width: 2px; height: 16px; border-radius: 2px; background: transparent; }.nav-group button:hover { background: #15191d; color: #dbe3de; }.nav-group button.active { background: #19211d; color: #e8f5ed; }.nav-group button.active::before { background: var(--console-accent); box-shadow: 0 0 10px rgba(84,234,164,.7); }.nav-group button .el-icon { width: 19px; flex: 0 0 19px; color: #707981; font-size: 15px; }.nav-group button.active .el-icon { color: var(--console-accent); }.nav-group button b { min-width: 19px; height: 18px; margin-left: auto; padding: 0 5px; display: grid; place-items: center; border-radius: 9px; background: #5a3326; color: #ffc1a5; font-size: 12px; }.collapsed .console-navigation { padding-inline: 8px; }.collapsed .nav-group button { padding: 0; justify-content: center; }.collapsed .nav-group button b { position: absolute; right: 1px; top: 1px; min-width: 13px; height: 13px; padding: 0 3px; }
.console-sidebar__footer { padding: 10px 8px; border-top: 1px solid var(--console-line); }.console-sidebar__footer > button { width: 100%; min-height: 36px; padding: 0 10px; display: flex; align-items: center; gap: 10px; border: 0; border-radius: 6px; background: transparent; color: #737c83; font: inherit; font-size: 12px; cursor: pointer; }.console-sidebar__footer > button:hover { background: #15191d; color: #dce3df; }.footer-tail { margin-left: auto; }
.sidebar-user { min-height: 50px; margin-top: 7px; padding: 8px 7px; display: grid; grid-template-columns: 31px 1fr 28px; gap: 8px; align-items: center; border-top: 1px solid #20252a; }.sidebar-user > span { width: 31px; height: 31px; display: grid; place-items: center; border-radius: 6px; background: #202832; color: #9ccff3; font-size: 12px; font-weight: 800; }.sidebar-user p { min-width: 0; display: flex; flex-direction: column; gap: 3px; }.sidebar-user strong { overflow: hidden; color: #d9e1dc; font-size: 12px; text-overflow: ellipsis; white-space: nowrap; }.sidebar-user small { color: #5d666d; font: 12px ui-monospace, monospace; }.sidebar-user button { width: 28px; height: 28px; display: grid; place-items: center; border: 0; border-radius: 5px; background: transparent; color: #606970; cursor: pointer; }.sidebar-user button:hover { background: #22272c; color: #e5ebe7; }
.console-main { min-width: 0; flex: 1; display: flex; flex-direction: column; }.console-topbar { height: 64px; flex: 0 0 64px; padding: 0 20px; display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid var(--console-line); background: rgba(13, 16, 19, .88); backdrop-filter: blur(16px); }.topbar-leading { min-width: 0; display: flex; align-items: center; gap: 12px; }.mobile-nav-trigger { width: 34px; height: 34px; display: none; place-items: center; border: 1px solid var(--console-line); border-radius: 6px; background: var(--console-panel); color: var(--console-ink); }.breadcrumb { min-width: 0; display: flex; align-items: center; gap: 8px; font-size: 12px; }.breadcrumb span, .breadcrumb i { color: #555e65; font-style: normal; }.breadcrumb strong { overflow: hidden; color: #cfd8d2; text-overflow: ellipsis; white-space: nowrap; }.topbar-actions { display: flex; align-items: center; gap: 8px; }.command-search { width: min(240px, 25vw); min-height: 34px; padding: 0 8px; display: flex; align-items: center; gap: 7px; border: 1px solid #30363b; border-radius: 6px; background: #13171a; color: #707980; font: inherit; font-size: 12px; cursor: pointer; }.command-search:hover { border-color: #485057; color: #aab3ad; }.command-search span { flex: 1; text-align: left; }.command-search kbd, .command-palette kbd { padding: 2px 5px; border: 1px solid #353c42; border-radius: 4px; background: #0d1012; color: #6e777e; font: 12px ui-monospace, monospace; }.runtime-pill { min-height: 34px; padding: 0 9px; display: flex; align-items: center; gap: 6px; border: 1px solid #292f34; border-radius: 6px; background: #111518; font-size: 12px; }.runtime-pill > i { width: 6px; height: 6px; border-radius: 50%; background: var(--console-danger); }.runtime-pill > i.up { background: var(--console-accent); box-shadow: 0 0 8px rgba(84,234,164,.6); }.runtime-pill > i.checking { background: var(--console-warning); }.runtime-pill span { color: #697279; }.runtime-pill b { color: #aeb8b1; font: 12px ui-monospace, monospace; }.runtime-pill em { width: 1px; height: 13px; background: #30363a; }
.notice-wrap { position: relative; }.notification-button { position: relative; width: 34px; height: 34px; display: grid; place-items: center; border: 1px solid #30363b; border-radius: 6px; background: #13171a; color: #8c969d; cursor: pointer; }.notification-button > i { position: absolute; right: 6px; top: 6px; width: 6px; height: 6px; border: 2px solid #13171a; border-radius: 50%; background: var(--console-warning); }.notice-popover { position: absolute; z-index: 60; top: 42px; right: 0; width: 310px; padding: 8px; border: 1px solid #343b41; border-radius: 8px; background: #121619; box-shadow: 0 22px 55px rgba(0,0,0,.42); }.notice-popover header { min-height: 38px; padding: 0 6px 5px 8px; display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid #282e33; color: #d7dfda; font-size: 12px; font-weight: 700; }.notice-popover header button { width: 28px; height: 28px; display: grid; place-items: center; border: 0; background: transparent; color: #6e777e; }.notice-item { width: 100%; min-height: 62px; margin-top: 5px; padding: 8px; display: grid; grid-template-columns: 34px 1fr auto; gap: 9px; align-items: center; border: 0; border-radius: 6px; background: #171c1f; color: #dce4df; font: inherit; text-align: left; cursor: pointer; }.notice-item:hover { background: #1c2420; }.notice-icon { width: 34px; height: 34px; display: grid; place-items: center; border-radius: 6px; background: #3b311d; color: #ffd17b; }.notice-item p { display: flex; flex-direction: column; gap: 4px; }.notice-item strong { font-size: 12px; }.notice-item small { color: #717a81; font-size: 12px; }.notice-item > b { color: #ffc76c; font-size: 12px; }.notice-item.muted .notice-icon { background: #1e2932; color: #88bde1; }.notice-item.muted > b { color: #667078; }
...
.command-backdrop { position: fixed; inset: 0; z-index: 100; padding-top: min(14vh, 120px); display: flex; justify-content: center; align-items: flex-start; background: rgba(0,0,0,.68); backdrop-filter: blur(4px); }.command-palette { width: min(620px, calc(100% - 28px)); max-height: min(620px, 72vh); display: flex; flex-direction: column; border: 1px solid #3d454b; border-radius: 8px; background: #101417; box-shadow: 0 35px 90px rgba(0,0,0,.62), 0 0 0 1px rgba(123,241,182,.04); overflow: hidden; }.command-palette > header { min-height: 58px; padding: 0 15px; display: flex; align-items: center; gap: 11px; border-bottom: 1px solid #2a3035; color: var(--console-accent); }.command-palette input { min-width: 0; flex: 1; border: 0; outline: 0; background: transparent; color: #edf3ef; font: inherit; font-size: 13px; }.command-palette input::placeholder { color: #5d666d; }.command-context { min-height: 38px; padding: 0 15px; display: flex; align-items: center; justify-content: space-between; color: #636c73; font: 12px ui-monospace, monospace; }.command-results { min-height: 0; padding: 0 7px 7px; overflow-y: auto; }.command-results > button { width: 100%; min-height: 57px; padding: 7px 9px; display: grid; grid-template-columns: 38px 1fr auto 18px; gap: 10px; align-items: center; border: 0; border-radius: 6px; background: transparent; color: #d8e0db; font: inherit; text-align: left; cursor: pointer; }.command-results > button:hover, .command-results > button:focus { outline: 0; background: #1a211d; }.command-icon { width: 36px; height: 36px; display: grid; place-items: center; border: 1px solid #30373c; border-radius: 6px; background: #151a1d; color: #8abfa6; }.command-results p { display: flex; flex-direction: column; gap: 4px; }.command-results strong { font-size: 12px; }.command-results small { color: #687179; font-size: 12px; }.command-results em { color: #697279; font-size: 12px; font-style: normal; }.command-results > button > .el-icon { color: #505960; }.command-empty { min-height: 220px; display: flex; flex-direction: column; align-items: center; justify-content: center; color: #5e676e; }.command-empty svg { width: 28px; }.command-empty strong { margin-top: 12px; color: #bbc4be; font-size: 12px; }.command-empty span { margin-top: 5px; font-size: 12px; }.command-palette > footer { min-height: 40px; padding: 0 14px; display: flex; align-items: center; gap: 18px; border-top: 1px solid #292f34; color: #616a71; font-size: 12px; }.command-palette > footer span { display: flex; align-items: center; gap: 4px; }
