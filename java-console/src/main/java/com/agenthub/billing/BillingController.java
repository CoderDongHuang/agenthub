...
public class BillingController {
    ...
    public BillingController(JdbcTemplate jdbc, CurrentUser currentUser) {
        this.jdbc = jdbc;
    ...
    public ApiResponse<Map<String, Object>> usage() {
        Map<String, Object> monthUsage = jdbc.queryForMap(
                "SELECT COALESCE(SUM(input_tokens),0) AS monthly_input, " +
                        ...
                        "COUNT(*) AS calls FROM token_usage WHERE tenant_id = ? AND created_at >= date_trunc('month', NOW())",
        ...
        List<Map<String, Object>> byAgent = jdbc.queryForList(
                "SELECT usage.agent_id, COALESCE(agent.name, 'Unknown Agent') AS agent_name, COUNT(*) AS calls, " +
                        ...
                        "WHERE usage.tenant_id = ? AND usage.created_at >= date_trunc('month', NOW()) " +
                        "GROUP BY usage.agent_id, agent.name ORDER BY total_cost DESC LIMIT 20", currentUser.tenantId()
        );
        List<Map<String, Object>> byModel = jdbc.queryForList(
                "SELECT model, COUNT(*) AS calls, SUM(input_tokens + output_tokens) AS total_tokens, " +
                        ...
                        "AND tenant_id = ? GROUP BY model ORDER BY total_cost DESC", currentUser.tenantId()
        );
        List<Map<String, Object>> daily = jdbc.queryForList(
                "WITH days AS (SELECT generate_series(CURRENT_DATE - INTERVAL '6 days', CURRENT_DATE, INTERVAL '1 day')::date AS day) " +
                        ...
                        "ON t.tenant_id = ? AND t.created_at >= days.day AND t.created_at < days.day + INTERVAL '1 day' " +
                        "GROUP BY days.day ORDER BY days.day", currentUser.tenantId()
        );
    ...
    public ApiResponse<Map<String, Object>> budget() {
        ...
        List<Map<String, Object>> budgets = jdbc.queryForList(
                "SELECT id, tenant_id, month, total_budget, alert_threshold FROM budget WHERE tenant_id = ? AND month = ?",
                currentUser.tenantId(), month
        );
        if (budgets.isEmpty()) {
    ...
    public ApiResponse<Map<String, Object>> updateBudget(@RequestBody Map<String, Object> body) {
        ...
        jdbc.update(
                "INSERT INTO budget (tenant_id, month, total_budget, alert_threshold) VALUES (?,?,?,?) " +
                        "ON CONFLICT (tenant_id, month) DO UPDATE SET total_budget = EXCLUDED.total_budget, " +
                        "alert_threshold = EXCLUDED.alert_threshold",
                currentUser.tenantId(), month, totalBudget, alertThreshold
        );
        return budget();
    }
}
